...
export default function DashboardLayout({
  ...
  return (
    <div style={{ background: 'var(--background)' }} className="h-screen flex overflow-hidden transition-all duration-500">
      ...
      <div className={`flex-1 flex flex-col h-screen transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'ml-0' : 'ml-0 md:ml-16 md:peer-hover:ml-56'} overflow-hidden`}>
        {/* Header — full width, dark navy */}
        <Header onMenuClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} />

        {/* Content panel — cream background */}
        <div style={{ backgroundColor: 'var(--panel-bg)' }} className="flex-1 flex flex-col overflow-hidden transition-all duration-500 panel-transition">
